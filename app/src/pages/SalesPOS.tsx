    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="card p-4">
        <form onSubmit={handleSubmit(onSearch)} className="flex gap-2">
          <input className="input" placeholder={t('app.search')!} {...register('query')} />
          <button className="btn btn-outline" type="submit">Go</button>
        </form>
        <div className="mt-4 grid grid-cols-2 md:grid-cols-3 gap-2">
          {products.map((p) => (
            <button key={p.id} className="card p-3 text-left hover:shadow-soft" onClick={() => addToCart(p)}>
              <div className="font-medium">{p.name}</div>
              <div className="text-sm text-muted">${p.price.toFixed(2)}</div>
      ...
      <div className="card p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="text-muted">Total: <span className="font-semibold">${total.toFixed(2)}</span></div>
        </div>
          ...
          {cart.map((l) => (
            <div key={l.id} className="flex items-center justify-between border-b border-neutral-200 dark:border-neutral-800 pb-2">
              <div>
                ...
                <div className="text-xs text-muted">x{l.qty} • ${l.price.toFixed(2)}</div>
              </div>
              <div className="flex items-center gap-2">
                <button className="btn btn-outline" onClick={() => setCart((prev)=>prev.map(x=>x.id===l.id?{...x, qty: Math.max(1,x.qty-1)}:x))}>-</button>
                <button className="btn btn-outline" onClick={() => setCart((prev)=>prev.map(x=>x.id===l.id?{...x, qty: x.qty+1}:x))}>+</button>
                <button className="btn btn-outline" onClick={() => setCart((prev)=>prev.filter(x=>x.id!==l.id))}>×</button>
              </div>
            </div>
          ))}
        </div>
        <button className="btn-primary w-full mt-4" onClick={checkout} disabled={!cart.length}>Checkout</button>
      </div>
    </div>
  )
}
